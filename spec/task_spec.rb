...
describe(Task) do
  ...
  describe('.not_done') do
    it("returns the task that are not done") do
      not_done_task1 = Task.create({:description => "get 'er done", :done => false})
      not_done_task2 = Task.create({:description => "gotta eat", :done => false})
...
describe("#list") do
  it('it tells which list it belongs to') do
